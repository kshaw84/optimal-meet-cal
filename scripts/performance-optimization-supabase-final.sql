-- Performance Optimization Script for Supabase (Final Version)
-- This script addresses the slow queries identified in the performance analysis
-- Modified for Supabase SQL Editor with correct column names
...
-- Create a view to monitor index usage (using correct column names)
...
-- Create a view to monitor table performance (using correct column names)
...
BEGIN
    ...
    SELECT 
        ...
        'Index ' || indexrelname || ' on ' || relname || ' has no scans'::text as description,
    ...
    SELECT 
        ...
        'Table ' || relname || ' may have bloat'::text as description,
        'medium'::text as severity
    FROM pg_stat_user_tables 
    ...
    SELECT 
        ...
        'Table ' || relname || ' has stale statistics'::text as description,
        'medium'::text as severity
    FROM pg_stat_user_tables 
