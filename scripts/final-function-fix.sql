-- Final Function Search Path Fix
-- This script drops and recreates ALL functions mentioned in the linter warnings
...
DROP FUNCTION IF EXISTS reprocess_routing_form_response_fields() CASCADE;
...
DROP FUNCTION IF EXISTS _process_routing_form_response_fields() CASCADE;
DROP FUNCTION IF EXISTS refresh_routing_form_response_denormalized() CASCADE;
DROP FUNCTION IF EXISTS refresh_routing_form_response_denormalized(integer) CASCADE;
...
DROP FUNCTION IF EXISTS calculate_is_team_booking() CASCADE;
DROP FUNCTION IF EXISTS refresh_booking_time_status_denormalized() CASCADE;
DROP FUNCTION IF EXISTS refresh_booking_time_status_denormalized(integer) CASCADE;
...
-- =============================================================================
-- 2. RECREATE ALL FUNCTIONS WITH SEARCH PATHS
-- =============================================================================
...
-- Check that all functions now have search paths
SELECT 
    'Functions with search paths' as status,
    count(*) as count
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE n.nspname = 'public' 
    AND p.prosrc LIKE '%SET search_path = public;%';
...
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE n.nspname = 'public' 
    AND p.prosrc NOT LIKE '%SET search_path = public;%'
