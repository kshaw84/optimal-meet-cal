-- Fix remaining functions without search paths

-- Fix reprocess_routing_form_response_fields function
...
-- Fix _process_routing_form_response_fields function
...
-- Verify the fix
SELECT 'Functions with search_path' as status, count(*) as count
FROM pg_proc p
...
SELECT 'Functions without search_path' as status, count(*) as count
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE n.nspname = 'public'
    AND p.prolang = (SELECT oid FROM pg_language WHERE lanname = 'plpgsql')
